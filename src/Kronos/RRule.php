...
class RRule {
	 ...
	 * @return bool True if the current recurrence rule is valid otherwise \Kronos\RRule\Exceptions\InvalidRRuleState exception is thrown.
	 */
	...
	private static function validateUntilDateString($date){
		...
		elseif($match === false){
			throw new \Exception('preg_match failed while trying this call: `preg_match(\'/^[0-9]{8}T[0-9]{6}Z$/\', "'.$date.'");`');
		}
		return true;
	}
}
